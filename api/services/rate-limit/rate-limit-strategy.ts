...
import { RateLimitResult, RateLimitRule } from '../../types/api';
...
export abstract class BaseRateLimitStrategy {
   ...
   */
  protected cleanupMetrics(maxAge: number = 3600000): void {
    // 1 hour default
    const now = Date.now();
...
export class SlidingWindowStrategy extends BaseRateLimitStrategy {
  ...
  async checkLimit(
    ...
    if (!allowed && validTimestamps.length > 0) {
      nextAllowedTime = (validTimestamps[0] || 0) + rule.window;
    }
