...
export async function handleAnalytics(request: Request, env: Env, path: string[]): Promise<Response> {
...
async function handleDashboard(
  ...
  // Calculate trends
  const requestsTrend = yesterdayStats.totalRequests > 0 
    ? ((todayStats.totalRequests - yesterdayStats.totalRequests) / yesterdayStats.totalRequests) * 100
    : 0;
    
  const responseTimeTrend = yesterdayStats.averageResponseTime > 0
