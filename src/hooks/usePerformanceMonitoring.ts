...
export function usePerformanceMonitoring(
  ...
  useEffect(() => {
    ...
    if (trackRenders) {
      ...
      if (renderCount.current === 1 && mountTime.current > 0) {
        ...
        logger.debug(`🎨 ${componentName} first render took ${timeSinceMount.toFixed(2)}ms`);
      }
    }
  });
  ...
  const measureTime = useCallback(
    (name: string, startMark: string, endMark?: string) => {
      ...
      const fullStartMark = startMark.includes(componentName)
        ? startMark
        : `${componentName}-${startMark}`;
      const fullEndMark = endMark
        ? endMark.includes(componentName)
          ? endMark
...
export function useFetchPerformanceMonitoring() {
  const trackFetch = useCallback(async (url: string, options?: RequestInit): Promise<Response> => {
...
export function useRenderPerformanceMonitoring(componentName: string, dependencies: any[] = []) {
  ...
  useEffect(() => {
    ...
    const timeSinceLastRender =
      lastRenderTime.current > 0 ? currentTime - lastRenderTime.current : 0;
    ...
    if (renderCount.current > 1) {
      ...
      if (timeSinceLastRender < 16) {
        ...
        console.warn(
          `⚡ ${componentName} is re-rendering very frequently (${timeSinceLastRender.toFixed(2)}ms since last render)`
        );
      }
    }
  }, dependencies);
