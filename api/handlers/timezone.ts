...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      timezoneRequest = {
        action: ['info', 'current', 'list', 'convert', 'difference'].includes(
...
function validateTimezoneRequest(request: TimezoneRequest): {
  valid: boolean;
  message?: string;
  details?: any;
} {
...
async function processTimezoneRequest(request: TimezoneRequest): Promise<any> {
  ...
  switch (request.action) {
    ...
    case 'convert':
      return await convertBetweenTimezones(timestamp, request.fromTimezone!, request.toTimezone!);
...
async function getTimezoneList(region?: string, search?: string): Promise<any> {
  ...
  // Filter by region
  if (region) {
    const regionKey = Object.keys(timezones).find(key =>
  ...
  if (search) {
    ...
    Object.keys(filteredTimezones).forEach(regionKey => {
      filteredTimezones[regionKey as keyof typeof filteredTimezones] = filteredTimezones[
        regionKey as keyof typeof filteredTimezones
  ...
  return {
    ...
    metadata: {
      ...
      totalTimezones: Object.values(enrichedTimezones).reduce(
        (sum: number, tzList) => sum + (Array.isArray(tzList) ? tzList.length : 0),
        0
      ),
...
async function convertBetweenTimezones(
  timestamp: number,
  fromTimezone: string,
  toTimezone: string
): Promise<any> {
