...
function compareResults(vercelResult, cloudflareResult, endpoint) {
  const comparison = {
    ...
    fasterPlatform: vercelResult.responseTime < cloudflareResult.responseTime ? 'vercel' : 'cloudflare'
  };
...
async function runMigrationTests() {
  ...
  for (const endpoint of allEndpoints) {
    ...
    console.log(`  Vercel: ${vercelStatus} ${vercelResult.status} (${vercelResult.responseTime}ms)`);
    console.log(`  Cloudflare: ${cloudflareStatus} ${cloudflareResult.status} (${cloudflareResult.responseTime}ms)`);
  ...
  console.log(`Compatible: ${compatibleTests}/${totalTests} (${Math.round(compatibleTests/totalTests*100)}%)`);
  console.log(`Vercel Success Rate: ${vercelSuccesses}/${totalTests} (${Math.round(vercelSuccesses/totalTests*100)}%)`);
  console.log(`Cloudflare Success Rate: ${cloudflareSuccesses}/${totalTests} (${Math.round(cloudflareSuccesses/totalTests*100)}%)`);

  // Performance comparison
  const avgVercelTime = results.reduce((sum, r) => sum + r.vercel.responseTime, 0) / totalTests;
  const avgCloudflareTime = results.reduce((sum, r) => sum + r.cloudflare.responseTime, 0) / totalTests;
  ...
  console.log(`Faster Platform: ${avgVercelTime < avgCloudflareTime ? 'Vercel' : 'Cloudflare'} (${Math.round(Math.abs(avgVercelTime - avgCloudflareTime))}ms difference)`);
  ...
  if (failedTests.length > 0) {
    ...
    failedTests.forEach(test => {
      console.log(`  - ${test.endpoint}: ${test.cloudflare.error || 'Status ' + test.cloudflare.status}`);
    });
  }
