...
const ENDPOINTS = [
  ...
  { url: 'https://tsconv.com/api/convert?timestamp=1640995200', type: 'api', description: 'Convert API (main domain)' },
  ...
  { url: 'https://api.tsconv.com/convert?timestamp=1640995200', type: 'api', description: 'Convert API (subdomain)' },
];
...
async function verifyMigration() {
  ...
  mainApiResults.forEach(result => {
    ...
    console.log(`  ${status} ${result.description}: ${result.status} ${jsonStatus} (${result.responseTime}ms)`);
  });

  // API results via subdomain
  const subdomainApiResults = results.filter(r => r.type === 'api' && r.url.includes('api.tsconv.com'));
  ...
  console.log('\n📈 Summary:');
  console.log(`  Total Tests: ${successfulTests}/${totalTests} passed (${Math.round(successfulTests/totalTests*100)}%)`);
  ...
  if (frontendWorking && mainApiWorking && subdomainApiWorking) {
    ...
    console.log('⚠️  MIGRATION MOSTLY COMPLETE - Main domain working, subdomain needs configuration');
