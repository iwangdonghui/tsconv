...
export default function CacheManagementDashboard() {
  ...
  const [_selectedEndpoint, _setSelectedEndpoint] = useState<string>(''); // TODO: Implement endpoint filtering
    ...
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>{health?.overall || 'Unknown'}</div>
                  ...
                  Object.entries(stats.endpoints).map(([endpoint, data]) => (
                    ...
                    >
                      ...
                      <div className='flex items-center gap-2'>
                        ...
                        <Button variant='outline' size='sm' onClick={() => clearCache(endpoint)}>
                          Clear
                        </Button>
                      </div>
                    </div>
                ...
                {health?.services &&
                  Object.entries(health.services).map(([service, data]) => (
                    ...
                    >
                      ...
                      <div className='flex items-center gap-2'>
                        <Badge className={getHealthColor(data.status)}>{data.status}</Badge>
                        {data.error && <span className='text-xs text-red-600'>{data.error}</span>}
                      </div>
                    </div>
                  ))}
              </div>
