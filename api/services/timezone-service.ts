...
class TimezoneService {
  ...
  private initializeCommonTimezones(): void {
    const commonZones = [
      { identifier: 'UTC', displayName: 'Coordinated Universal Time', region: 'Global', popularityRank: 1 },
      { identifier: 'America/New_York', displayName: 'Eastern Time (US & Canada)', region: 'North America', popularityRank: 2 },
      { identifier: 'America/Chicago', displayName: 'Central Time (US & Canada)', region: 'North America', popularityRank: 3 },
      { identifier: 'America/Denver', displayName: 'Mountain Time (US & Canada)', region: 'North America', popularityRank: 4 },
      { identifier: 'America/Los_Angeles', displayName: 'Pacific Time (US & Canada)', region: 'North America', popularityRank: 5 },
      { identifier: 'America/Toronto', displayName: 'Eastern Time (Canada)', region: 'North America', popularityRank: 6 },
      { identifier: 'America/Vancouver', displayName: 'Pacific Time (Canada)', region: 'North America', popularityRank: 7 },
      ...
      { identifier: 'Asia/Singapore', displayName: 'Singapore', region: 'Asia', popularityRank: 16 },
      ...
      { identifier: 'Australia/Sydney', displayName: 'Sydney', region: 'Australia', popularityRank: 19 },
      { identifier: 'Australia/Melbourne', displayName: 'Melbourne', region: 'Australia', popularityRank: 20 },
      { identifier: 'Pacific/Auckland', displayName: 'Auckland', region: 'Pacific', popularityRank: 21 },
      { identifier: 'Brazil/East', displayName: 'Brasília', region: 'South America', popularityRank: 22 },
      { identifier: 'America/Sao_Paulo', displayName: 'São Paulo', region: 'South America', popularityRank: 23 },
      { identifier: 'Africa/Johannesburg', displayName: 'Johannesburg', region: 'Africa', popularityRank: 24 },
  ...
  getTimezoneOffset(date: Date, timezone: string): number {
    try {
      ...
      throw new Error(`Failed to get timezone offset for ${timezone}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private findDSTTransition(year: number, timezone: string, season: 'spring' | 'fall'): {
    date: Date;
    offsetBefore: number;
    offsetAfter: number;
  } | null {
  ...
  searchTimezones(query: string): CommonTimezone[] {
    ...
    return this.commonTimezones.filter(tz => {
      ...
      if (identifier.includes(lowerQuery) || 
      ...
      return queryWords.every(word => 
        identifierParts.some(part => part.includes(word))
      );
    });
  }
  ...
  getTimezoneSuggestions(input: string): CommonTimezone[] {
    ...
    const suggestions = this.commonTimezones.filter(tz => 
  ...
  calculateTimezoneOffsetDifference(fromTimezone: string, toTimezone: string, date?: Date): number {
    try {
      ...
      throw new Error(`Failed to calculate timezone offset difference: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  getTimezoneOffsetAtDate(timezone: string, date: Date): {
    offset: number;
    isDST: boolean;
    displayName: string;
  } {
    try {
      ...
      throw new Error(`Failed to get timezone offset at date: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private isDSTTransitionTime(date: Date, timezone: string): boolean {
    try {
      ...
      return transitions.some(transition => 
        transition.date.split('T')[0] === dateStr
      );
    } catch (error) {
      return false;
    }
