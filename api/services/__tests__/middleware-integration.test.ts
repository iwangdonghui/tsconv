...
describe('Middleware Integration Tests', () => {
  ...
  describe('Cache + Rate Limit Integration', () => {
    ...
    it('should prevent cache access when rate limited', async () => {
      ...
      for (let i = 0; i < 105; i++) {
        ...
        await rateLimitMiddleware(
          requests[i] as VercelRequest,
          freshRes as VercelResponse,
          nextFunction
        );
      }
      ...
      await rateLimitMiddleware(
        rateLimitedReq as VercelRequest,
        testRes as VercelResponse,
        nextFunction
      );
    ...
    it('should cache rate limit information', async () => {
      ...
      const rateLimitMiddlewareTest = async (
        req: VercelRequest,
        res: VercelResponse,
        next: Function
      ) => {
    ...
    it('should respect cache-control headers', async () => {
      ...
      const cacheMiddlewareTest = async (
        req: VercelRequest,
        res: VercelResponse,
        next: Function
      ) => {
  ...
  describe('Performance Under Load', () => {
    it('should handle concurrent requests efficiently', async () => {
      ...
      const successfulRequests = results.filter(
        r => !r.res.status.mock?.calls?.some(call => call[0] === 429)
      );
  ...
  describe('Security and Validation', () => {
    ...
    it('should sanitize error messages', async () => {
      ...
      cacheService.get = vi
        .fn()
        .mockRejectedValue(new Error('Database password is invalid: abc123'));
