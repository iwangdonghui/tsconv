    ...
    test: async () => {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        results.push({
          ...
          hasRateLimitHeaders: !!(result.headers['x-ratelimit-limit'] || result.headers['X-RateLimit-Limit'])
        });
      }
      return results;
    }
...
async function runProductionOptimizationTests() {
  ...
  console.log(`Analytics Tests: ${analyticsResults.passedTests}/${analyticsResults.totalTests} passed (${Math.round(analyticsResults.passedTests/analyticsResults.totalTests*100)}%)`);
  console.log(`Security Tests: ${securityResults.filter(r => r.success).length}/${securityResults.length} passed (${Math.round(securityResults.filter(r => r.success).length/securityResults.length*100)}%)`);
