...
import { _FormatValidationResult } from '../../types/api';
...
describe('Format Service', () => {
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle date formatting at DST transitions', async () => {
      ...
      const results = await formatService.format(
        dstTransitionTimestamp,
        formats,
        'America/New_York'
      );
      expect(results[0].success).toBe(true);
  ...
  describe('Format Value Validation', () => {
    ...
    it('should validate complex format patterns correctly', () => {
      // RFC 2822
      const validRfc = formatService.validateFormatValue(
        'Mon, 01 Jan 2021 00:00:00 +0000',
        'rfc2822'
      );
      expect(validRfc.valid).toBe(true);

      const invalidRfc = formatService.validateFormatValue(
        'Monday, 01 Jan 2021 00:00:00',
        'rfc2822'
      );
  ...
  describe('Regex Pattern Compilation', () => {
    it('should compile regex patterns correctly for all formats', () => {
      ...
      for (const format of formats) {
        ...
        if (formatObj) {
          ...
          const validation = formatService.validateFormat(
            format.name.toLowerCase().replace(/\s+/g, '-')
          );
    ...
    it('should validate format values with compiled regex patterns', () => {
      const testCases = [
        ...
        {
          format: 'sql-datetime',
          validValue: '2024-01-15 14:30:00',
          invalidValue: '2024-01-15T14:30:00',
        },
      ...
      for (const testCase of testCases) {
        ...
        const invalidResult = formatService.validateFormatValue(
  ...
  describe('Performance Tests', () => {
    ...
    it('should handle concurrent format operations efficiently', async () => {
      ...
      await Promise.all(formatsList.map(formats => formatService.format(timestamp, formats)));
    ...
    it('should handle large batch of format operations', async () => {
      ...
      await Promise.all(timestamps.map(timestamp => formatService.format(timestamp, formats)));
